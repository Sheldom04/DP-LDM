...
if __name__ == "__main__":
    ...
    # diffusion.netG.vae_lode_dict("my_ldm_concat_cond/my_pt/best/vae_epoch_150-1-1.pth")
    ...
    for _,  val_data in enumerate(val_loader):
        ...
        visuals = diffusion.get_current_visuals(need_LR=False, sample=False)
        ...
        if sr_img_mode == 'single':
            # single img series
            sr_img = visuals['SAM']  # uint8
            sample_num = sr_img.shape[0]
            for iter in range(0, sample_num):
                new_filename = f"{idx:06d}.png"
                Metrics.save_img(
                    Metrics.tensor2img(sr_img[iter]), new_filename)
            ...
            new_filename = f"{result_path}/{idx:06d}.png"
            Metrics.save_img(
                Metrics.tensor2img(visuals['SR']), new_filename)
