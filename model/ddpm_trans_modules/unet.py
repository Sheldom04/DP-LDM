# 开发者：SHELDOM
# 开发时间: 2024/7/24 17:03
import math
import torch
from torch import nn
from inspect import isfunction
from VGG.DC_network import VGG16
from model.ddpm_trans_modules.trans_block_eca import TransformerBlock_eca, TransformerBlock_ema, TransformerBlock_ca
...
class ResnetBloc_eca(nn.Module):  # 增加注意力机制
...
class ResnetBloc_ca(nn.Module):  # 增加注意力机制
    def __init__(self, dim, dim_out, *, time_emb_dim=None, norm_groups=32, dropout=0, with_attn=False):
        super().__init__()
        self.with_attn = with_attn
        self.res_block = ResnetBlock(
            dim, dim_out, time_emb_dim, norm_groups=norm_groups, dropout=dropout)
        if with_attn:
            self.attn = nn.Sequential(*[TransformerBlock_ca(dim=int(dim), num_heads=2, ffn_expansion_factor=2.66,
                                                             bias=False, LayerNorm_type='WithBias') for i in range(1)])
...
class ResnetBloc_ema(nn.Module):  # 增加ema注意力机制
    def __init__(self, dim, dim_out, *, time_emb_dim=None, norm_groups=32, dropout=0, with_attn=False):
        super().__init__()
        self.with_attn = with_attn
        self.res_block = ResnetBlock(
            dim, dim_out, time_emb_dim, norm_groups=norm_groups, dropout=dropout)
        if with_attn:
            self.attn = nn.Sequential(*[TransformerBlock_ema(dim=int(dim), num_heads=2, ffn_expansion_factor=2.66,
                                                             bias=False, LayerNorm_type='WithBias') for i in range(1)])
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(dim, dim // 2, kernel_size=3, stride=1, padding=1),
                                   nn.PixelUnshuffle(2))
        # PixelUnshuffle(2)用于下采样 长宽分辨减少到1/2  通道数增加为4倍
        ...
        self.block1 = ResnetBloc_ca(dim=dim, dim_out=dim, time_emb_dim=time_dim, norm_groups=norm_groups,
                                     with_attn=True)
        self.block2 = ResnetBloc_ca(dim=dim * 2 ** 1, dim_out=dim * 2 ** 1, time_emb_dim=time_dim,
                                     norm_groups=norm_groups, with_attn=True)
        self.block3 = ResnetBloc_ca(dim=dim * 2 ** 2, dim_out=dim * 2 ** 2, time_emb_dim=time_dim,
                                     norm_groups=norm_groups, with_attn=True)
        self.block4 = ResnetBloc_ca(dim=dim * 2 ** 3, dim_out=dim * 2 ** 3, time_emb_dim=time_dim,
                                     norm_groups=norm_groups, with_attn=True)
        ...
        self.decoder_block3 = ResnetBloc_ca(dim=dim * 2 ** 2, dim_out=dim * 2 ** 2, time_emb_dim=time_dim,
                                             norm_groups=norm_groups, with_attn=True)
        self.decoder_block2 = ResnetBloc_ca(dim=dim * 2 ** 1, dim_out=dim * 2 ** 1, time_emb_dim=time_dim,
                                             norm_groups=norm_groups, with_attn=True)
        self.decoder_block1 = ResnetBloc_ca(dim=dim * 2 ** 1, dim_out=dim * 2 ** 1, time_emb_dim=time_dim,
                                             norm_groups=norm_groups, with_attn=True)


    def forward(self, x, t, x_start):
        # Yl, Yh = self.dwt(x_start)
        vgg1 = self.vgg.block1(x_start)
        vgg2 = self.vgg.block2(vgg1)
        vgg3 = self.vgg.block3(vgg2)
        vgg4 = self.vgg.block4(vgg3)
        vgg5 = self.vgg.block5(vgg4)
        # vgg6 = self.vgg.block6(vgg5)
        ...
        de_level3 = torch.cat([de_level3, x3], 1)  # 增加维度
        de_level3 = self.conv_cat3(de_level3)  # 减少维度
...
class UNet(nn.Module):
    def __init__(
        ...
        self.refine = ResnetBloc_ca(dim=dim * 2 ** 1, dim_out=dim * 2 ** 1, time_emb_dim=time_dim,
                                     norm_groups=norm_groups, with_attn=True)
        self.de_predict = nn.Sequential(nn.Conv2d(dim * 2 ** 1, out_channel, kernel_size=1, stride=1))

    def forward(self, x, time, x_statr):
        # print(time.shape)
        t = self.time_mlp(time) if exists(self.time_mlp) else None  # 时间编码

        mid_feat, x1 = self.encoder_water(x, t, x_statr)
...
if __name__ == '__main__':
    img = torch.zeros(2, 6, 64, 64)
    x_start = torch.zeros(2, 3, 256, 256)
    time = torch.tensor([1, 2])  # batch_size 为2 所以要两个t
    model = UNet()
    output = model(img, time, x_start)
    # output = model2(img)
    print(output.shape)
    # print(a.shape)
    # print(b)
